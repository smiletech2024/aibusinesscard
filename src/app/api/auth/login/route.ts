...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── 現在の試行記録を取得（テーブル未作成でも続行） ─────────
    let record: { attempts: number; locked_until: string | null; last_attempt: string } | null = null
    try {
      ...
      // login_attempts テーブル未作成の場合はスキップ
    }
    ...
    if (authError || !authData.session) {
      ...
      try {
        ...
        // テーブル未作成の場合は単純に401
        return NextResponse.json({ error: 'invalid_credentials', attempts: 1, maxAttempts: MAX_ATTEMPTS, remaining: MAX_ATTEMPTS - 1 }, { status: 401 })
      }
    }
