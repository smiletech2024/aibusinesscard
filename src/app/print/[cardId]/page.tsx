...
/* 各プリセットの初期配置 */
const LAYOUT_PRESETS: Record<string, PrintEls> = {
  standard: DEFAULT_PRINT_ELS,
  centered: {
    company:    { x: 130, y: 52,  w: 300, h: 28  },
    name_block: { x: 90,  y: 86,  w: 380, h: 76  },
    contacts:   { x: 120, y: 172, w: 320, h: 88  },
    qr_block:   { x: 416, y: 50,  w: 122, h: 130 },
  },
  split: {
    company:    { x: 228, y: 22,  w: 220, h: 28  },
    name_block: { x: 18,  y: 60,  w: 185, h: 200 },
    contacts:   { x: 228, y: 58,  w: 220, h: 110 },
    qr_block:   { x: 352, y: 188, w: 110, h: 120 },
  },
  pulse: {
    company:    { x: 240, y: 22,  w: 220, h: 28  },
    name_block: { x: 240, y: 58,  w: 220, h: 76  },
    contacts:   { x: 240, y: 148, w: 220, h: 88  },
    qr_block:   { x: 18,  y: 38,  w: 130, h: 150 },
  },
}
...
function FrontBrandMark({ theme }: { theme: 'light' | 'dark' }) {
  ...
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
      <LogoIcon size={13} />
      <span style={{ fontSize: 18, fontWeight: 700, color, letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>AI名刺</span>
    </div>
  )
}
  ...
  return (
    <div className="print-card" style={{ width: W, height: H, background: '#FFFFFF', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      <div style={{ position: 'absolute', left: 32, top: 0, right: 112, bottom: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 0 }}>
        ...
        {/* Company */}
        {card.company && (
          <p style={{ fontSize: 18, color: '#F26722', fontWeight: 700, letterSpacing: '0.15em', margin: '0 0 8px' }}>
            {card.company}
          </p>
        )}
        ...
        {/* Title */}
        {card.title && (
          <p style={{ fontSize: 18, color: '#F26722', fontWeight: 600, margin: '0 0 20px', letterSpacing: '0.02em' }}>
            {card.title}
          </p>
        )}
      ...
      {qrUrl && (
        <div style={{ position: 'absolute', right: 20, bottom: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 2 }}>
            <LogoIcon size={12} />
            <span style={{ fontSize: 18, fontWeight: 800, color: '#F26722', letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>AI名刺</span>
          </div>
          <QRCodeSVG url={qrUrl} size={76} style={{ border: '1.5px solid #E0E7FF', borderRadius: 8 }} />
          <p style={{ fontSize: 18, color: '#A5B4FC', margin: 0, fontWeight: 700, letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>スキャンして分身AIに相談</p>
        </div>
      )}
  ...
  return (
    <div className="print-card" style={{ width: W, height: H, background: '#1C0F05', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      <div style={{ position: 'absolute', left: 40, top: 0, right: 40, bottom: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <p style={{ fontSize: 18, color: '#F5A47A', fontWeight: 700, letterSpacing: '0.15em', margin: '0 0 8px', lineHeight: 1.2 }}>
          分身AI搭載名刺
        </p>
        <h3 style={{ fontSize: 18, fontWeight: 900, color: 'white', margin: '0 0 10px', lineHeight: 1.3, letterSpacing: '-0.01em' }}>
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(242,103,34,0.2)', borderRadius: 20, padding: '6px 14px', border: '1px solid rgba(242,103,34,0.3)' }}>
          ...
          <span style={{ fontSize: 18, color: '#A5B4FC', fontWeight: 700, lineHeight: 1.2 }}>24時間 オンライン対応中</span>
        </div>
      </div>
    </div>
  )
    ...
    <div className="print-card" style={{ width: W, height: H, background: '#0D0C2A', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      <div style={{ position: 'absolute', left: 32, top: 36, bottom: 36, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        <div>
          ...
          {card.title && (
            <p style={{ fontSize: 18, background: 'linear-gradient(90deg, #FDD5B5, #F5C09A)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontWeight: 600, margin: 0 }}>
              {card.title}
            </p>
          )}
        </div>
      ...
      {qrUrl && (
        <div style={{ position: 'absolute', right: 22, bottom: 18, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 2 }}>
            <LogoIcon size={12} />
            <span style={{ fontSize: 18, fontWeight: 800, color: '#F5C09A', letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>AI名刺</span>
          </div>
          ...
          <p style={{ fontSize: 18, color: '#F5A47A', margin: 0, fontWeight: 700, letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>スキャンして分身AIに相談</p>
        </div>
      )}
...
function MidnightBack({ card, fontFamily }: { card: BusinessCard; fontFamily?: string }) {
  return (
    <div className="print-card" style={{ width: W, height: H, background: 'linear-gradient(135deg, #0D0C2A 0%, #150E3A 50%, #0D0C2A 100%)', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      {/* Center content */}
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px 36px', textAlign: 'center' }}>
        <div style={{ marginBottom: 10 }}>
          <LogoIcon size={32} />
        </div>

        <p style={{ fontSize: 18, color: '#F5A47A', fontWeight: 700, letterSpacing: '0.18em', margin: '0 0 8px', lineHeight: 1.2 }}>分身AI搭載名刺</p>
        <h3 style={{ fontSize: 18, fontWeight: 900, color: 'white', margin: '0 0 12px', lineHeight: 1.3, letterSpacing: '-0.01em' }}>
          QRをスキャンして<br />分身AIと話してください
        </h3>
        <BackFeatureList accent="#F5A47A" />
        <div style={{ height: 1, width: 60, background: 'linear-gradient(90deg, transparent, #F5A47A, transparent)', marginTop: 8 }} />
      </div>
    </div>
  )
}
    ...
    <div className="print-card" style={{ width: W, height: H, background: 'linear-gradient(135deg, #C4511A 0%, #F26722 40%, #D4691E 70%, #F59340 100%)', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
        ...
        <div>
          ...
          {card.title && (
            <div style={{ display: 'inline-block', background: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(10px)', borderRadius: 4, padding: '3px 10px', border: '1px solid rgba(255,255,255,0.2)' }}>
              <p style={{ fontSize: 18, color: 'rgba(255,255,255,0.95)', fontWeight: 600, margin: 0 }}>
                {card.title}
              </p>
            </div>
          )}
      ...
      {qrUrl && (
        <div style={{ position: 'absolute', right: 20, bottom: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 2 }}>
            <LogoIcon size={12} />
            <span style={{ fontSize: 18, fontWeight: 800, color: 'rgba(255,255,255,0.7)', letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>AI名刺</span>
          </div>
          ...
          <p style={{ fontSize: 18, color: 'rgba(255,255,255,0.6)', margin: 0, fontWeight: 700, letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>スキャンして分身AIに相談</p>
        </div>
      )}
      ...
      <div style={{ position: 'absolute', bottom: 14, left: 20, display: 'flex', alignItems: 'center', gap: 5 }}>
        ...
        <span style={{ fontSize: 18, color: 'rgba(255,255,255,0.5)', fontWeight: 600, whiteSpace: 'nowrap' }}>分身AI オンライン</span>
      </div>
    </div>
  )
}
...
function VividBack({ card, fontFamily }: { card: BusinessCard; fontFamily?: string }) {
  return (
    <div className="print-card" style={{ width: W, height: H, background: 'linear-gradient(225deg, #C4511A 0%, #F26722 30%, #F59340 70%, #F59340 100%)', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      {/* Content */}
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px 40px', textAlign: 'center' }}>
        <div style={{ marginBottom: 10 }}>
          <LogoIcon size={32} />
        </div>
        <p style={{ fontSize: 18, color: 'rgba(255,255,255,0.8)', fontWeight: 700, letterSpacing: '0.18em', margin: '0 0 8px', lineHeight: 1.2 }}>分身AI搭載名刺</p>
        <h3 style={{ fontSize: 18, fontWeight: 900, color: 'white', margin: '0 0 10px', lineHeight: 1.3, textShadow: '0 2px 10px rgba(0,0,0,0.2)' }}>
          このQRから<br />分身AIに相談できます
        </h3>
        <BackFeatureList accent="#FFFFFF" />
      </div>
    </div>
  )
}
  ...
  return (
    <div className="print-card" style={{ width: W, height: H, background: 'linear-gradient(135deg, #0B2A4A 0%, #0B4F7A 50%, #0D7A7A 100%)', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      {qrUrl && (
        <div style={{ position: 'absolute', right: 20, bottom: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 2 }}>
            <LogoIcon size={12} /><span style={{ fontSize: 18, fontWeight: 800, color: '#7EF2E8', letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>AI名刺</span>
          </div>
          <div style={{ background: 'white', padding: 5, borderRadius: 10, boxShadow: '0 0 20px rgba(13,212,200,0.4)' }}>
            <QRCodeSVG url={qrUrl} size={68} />
          </div>
          <p style={{ fontSize: 18, color: '#7EF2E8', margin: 0, fontWeight: 700, letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>スキャンして分身AIに相談</p>
        </div>
      )}
...
function OceanBack({ card, fontFamily }: { card: BusinessCard; fontFamily?: string }) {
  return (
    <div className="print-card" style={{ width: W, height: H, background: 'linear-gradient(160deg, #071828 0%, #0B2A4A 60%, #0B4F7A 100%)', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px 36px', textAlign: 'center' }}>
        <div style={{ marginBottom: 10 }}><LogoIcon size={32} /></div>
        <p style={{ fontSize: 18, color: '#7EF2E8', fontWeight: 700, letterSpacing: '0.18em', margin: '0 0 8px', lineHeight: 1.2 }}>分身AI搭載名刺</p>
        <h3 style={{ fontSize: 18, fontWeight: 900, color: 'white', margin: '0 0 12px', lineHeight: 1.3 }}>QRをスキャンして<br />分身AIと話してください</h3>
        <BackFeatureList accent="#7EF2E8" />
        <div style={{ height: 1, width: 60, background: 'linear-gradient(90deg, transparent, #7EF2E8, transparent)' }} />
      </div>
    </div>
  )
}
  ...
  return (
    <div className="print-card" style={{ width: W, height: H, background: 'linear-gradient(135deg, #0A2A1C 0%, #0F3D2E 50%, #145A3E 100%)', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      {qrUrl && (
        <div style={{ position: 'absolute', right: 20, bottom: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 2 }}>
            <LogoIcon size={12} /><span style={{ fontSize: 18, fontWeight: 800, color: '#6EE7B7', letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>AI名刺</span>
          </div>
          <div style={{ background: 'white', padding: 5, borderRadius: 10, boxShadow: '0 0 18px rgba(52,211,153,0.35)' }}>
            <QRCodeSVG url={qrUrl} size={68} />
          </div>
          <p style={{ fontSize: 18, color: '#6EE7B7', margin: 0, fontWeight: 700, letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>スキャンして分身AIに相談</p>
        </div>
      )}
...
function ForestBack({ card, fontFamily }: { card: BusinessCard; fontFamily?: string }) {
  return (
    <div className="print-card" style={{ width: W, height: H, background: 'linear-gradient(160deg, #061610 0%, #0A2A1C 60%, #0F3D2E 100%)', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px 36px', textAlign: 'center' }}>
        <div style={{ marginBottom: 10 }}><LogoIcon size={32} /></div>
        <p style={{ fontSize: 18, color: '#6EE7B7', fontWeight: 700, letterSpacing: '0.18em', margin: '0 0 8px', lineHeight: 1.2 }}>分身AI搭載名刺</p>
        <h3 style={{ fontSize: 18, fontWeight: 900, color: 'white', margin: '0 0 12px', lineHeight: 1.3 }}>QRをスキャンして<br />分身AIと話してください</h3>
        <BackFeatureList accent="#6EE7B7" />
        <div style={{ height: 1, width: 60, background: 'linear-gradient(90deg, transparent, #6EE7B7, transparent)' }} />
      </div>
    </div>
  )
}
    ...
    <div className="print-card" style={{ width: W, height: H, background: 'linear-gradient(135deg, #3D0010 0%, #7A0B2A 50%, #A01040 100%)', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      {qrUrl && (
        <div style={{ position: 'absolute', right: 22, bottom: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 2 }}>
            <LogoIcon size={12} /><span style={{ fontSize: 18, fontWeight: 800, color: '#FECDD3', letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>AI名刺</span>
          </div>
          <div style={{ background: 'white', padding: 5, borderRadius: 10, boxShadow: '0 0 18px rgba(244,63,94,0.4)' }}>
            <QRCodeSVG url={qrUrl} size={68} />
          </div>
          <p style={{ fontSize: 18, color: '#FECDD3', margin: 0, fontWeight: 700, letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>スキャンして分身AIに相談</p>
        </div>
      )}
...
function CrimsonBack({ card, fontFamily }: { card: BusinessCard; fontFamily?: string }) {
  return (
    <div className="print-card" style={{ width: W, height: H, background: 'linear-gradient(160deg, #1C0008 0%, #3D0010 60%, #7A0B2A 100%)', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px 36px', textAlign: 'center' }}>
        <div style={{ marginBottom: 10 }}><LogoIcon size={32} /></div>
        <p style={{ fontSize: 18, color: '#FECDD3', fontWeight: 700, letterSpacing: '0.18em', margin: '0 0 8px', lineHeight: 1.2 }}>分身AI搭載名刺</p>
        <h3 style={{ fontSize: 18, fontWeight: 900, color: 'white', margin: '0 0 12px', lineHeight: 1.3 }}>QRをスキャンして<br />分身AIと話してください</h3>
        <BackFeatureList accent="#FECDD3" />
        <div style={{ height: 1, width: 60, background: 'linear-gradient(90deg, transparent, #FECDD3, transparent)' }} />
      </div>
    </div>
  )
}
    ...
    <div className="print-card" style={{ width: W, height: H, background: '#0A0A0A', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      {qrUrl && (
        <div style={{ position: 'absolute', right: 18, bottom: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 2 }}>
            <LogoIcon size={12} /><span style={{ fontSize: 18, fontWeight: 800, color: '#D4AF37', letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>AI名刺</span>
          </div>
          <div style={{ background: 'white', padding: 5, borderRadius: 10, boxShadow: '0 0 20px rgba(212,175,55,0.4)' }}>
            <QRCodeSVG url={qrUrl} size={68} />
          </div>
          <p style={{ fontSize: 18, color: '#D4AF37', margin: 0, fontWeight: 700, letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>スキャンして分身AIに相談</p>
        </div>
      )}
...
function GoldBack({ card, fontFamily }: { card: BusinessCard; fontFamily?: string }) {
  return (
    <div className="print-card" style={{ width: W, height: H, background: '#050505', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px 36px', textAlign: 'center' }}>
        <div style={{ marginBottom: 10 }}><LogoIcon size={32} /></div>
        <p style={{ fontSize: 18, color: '#D4AF37', fontWeight: 700, letterSpacing: '0.22em', margin: '0 0 8px', lineHeight: 1.2 }}>分身AI搭載名刺</p>
        <h3 style={{ fontSize: 18, fontWeight: 900, color: '#F5E6A3', margin: '0 0 12px', lineHeight: 1.3 }}>QRをスキャンして<br />分身AIと話してください</h3>
        <BackFeatureList accent="#D4AF37" textColor="rgba(245,230,163,0.85)" />
        <div style={{ height: 1, width: 60, background: 'linear-gradient(90deg, transparent, #D4AF37, transparent)', marginTop: 8 }} />
      </div>
    </div>
  )
}
    ...
    <div className="print-card" style={{ width: W, height: H, background: 'linear-gradient(135deg, #FDF2F8 0%, #FCE7F3 50%, #FBD5EA 100%)', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      {qrUrl && (
        <div style={{ position: 'absolute', right: 18, bottom: 14, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 2 }}>
            <LogoIcon size={12} /><span style={{ fontSize: 18, fontWeight: 800, color: '#BE185D', letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>AI名刺</span>
          </div>
          <QRCodeSVG url={qrUrl} size={74} style={{ border: '1.5px solid #FBCFE8', borderRadius: 8 }} />
          <p style={{ fontSize: 18, color: '#F472B6', margin: 0, fontWeight: 700, letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>スキャンして分身AIに相談</p>
        </div>
      )}
...
function PinkBack({ card, fontFamily }: { card: BusinessCard; fontFamily?: string }) {
  return (
    <div className="print-card" style={{ width: W, height: H, background: 'linear-gradient(160deg, #831843 0%, #9D174D 40%, #BE185D 100%)', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px 36px', textAlign: 'center' }}>
        <div style={{ marginBottom: 10 }}><LogoIcon size={32} /></div>
        <p style={{ fontSize: 18, color: '#FBCFE8', fontWeight: 700, letterSpacing: '0.18em', margin: '0 0 8px', lineHeight: 1.2 }}>分身AI搭載名刺</p>
        <h3 style={{ fontSize: 18, fontWeight: 900, color: 'white', margin: '0 0 12px', lineHeight: 1.3 }}>QRをスキャンして<br />分身AIと話してください</h3>
        <BackFeatureList accent="#F9A8D4" />
        <div style={{ height: 1, width: 60, background: 'linear-gradient(90deg, transparent, #F9A8D4, transparent)' }} />
      </div>
    </div>
  )
}
      ...
      {qrUrl && (
        <div style={{ position: 'absolute', right: 18, bottom: 14, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 3, marginBottom: 2 }}>
            <LogoIcon size={11} /><span style={{ fontSize: 18, fontWeight: 800, color: accent, letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>AI名刺</span>
          </div>
          <div style={qrWrap}><QRCodeSVG url={qrUrl} size={64} /></div>
          <p style={{ fontSize: 18, color: accent, margin: 0, fontWeight: 700, whiteSpace: 'nowrap' }}>スキャンして分身AIに相談</p>
        </div>
      )}
    </div>
  )
    ...
    <div className="print-card" style={{ width: W, height: H, background: rightBg, position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: SPLIT_X, background: panelBg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', textAlign: 'center', padding: '0 14px' }}>
          ...
          {card.title && <p style={{ fontSize: 18, color: 'rgba(255,255,255,0.6)', margin: '6px 0 0', fontWeight: 600, lineHeight: 1.4 }}>{card.title}</p>}
        </div>
        {/* パネル下部ブランドマーク */}
        <div style={{ position: 'absolute', bottom: 12, display: 'flex', alignItems: 'center', gap: 4 }}>
          <LogoIcon size={10} />
          <span style={{ fontSize: 18, color: 'rgba(255,255,255,0.4)', fontWeight: 700 }}>AI名刺</span>
        </div>
      </div>
      {/* 右パネル */}
  ...
  return (
    <div className="print-card" style={{ width: W, height: H, background: '#060614', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      <div style={{ position: 'absolute', top: 14, left: 20, display: 'flex', alignItems: 'center', gap: 5 }}>
        ...
        <span style={{ fontSize: 18, color: 'rgba(255,255,255,0.45)', fontWeight: 800, letterSpacing: '0.16em' }}>AI LIVE</span>
      </div>
      {/* 会社名 */}
      {card.company && (
        <div style={{ position: 'absolute', top: 13, right: 18 }}>
          <span style={{ fontSize: 18, color: accent, fontWeight: 700, letterSpacing: '0.16em', opacity: 0.9 }}>{card.company}</span>
        </div>
      )}
      ...
      <div style={{ position: 'absolute', left: 16, top: 0, bottom: 0, width: 200, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          <span style={{ fontSize: 18, color: accent, fontWeight: 900, letterSpacing: '0.14em' }}>SCAN</span>
          <svg width="14" height="8" viewBox="0 0 14 8" fill="none">
            <path d="M0 4h12M9 1l3 3-3 3" stroke={accent} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          <span style={{ fontSize: 18, color: 'rgba(255,255,255,0.4)', fontWeight: 600, letterSpacing: '0.06em' }}>分身AIと対話</span>
        </div>
      </div>
      ...
      <div style={{ position: 'absolute', left: 236, right: 18, top: 0, bottom: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        {/* 役職 */}
        {card.title && (
          <p style={{ fontSize: 18, color: accent, fontWeight: 700, margin: '0 0 16px', letterSpacing: '0.02em' }}>{card.title}</p>
        )}
        {/* EKG 鼓動ライン */}
...
function PulseBack({ card, fontFamily, tc }: { card: BusinessCard; fontFamily?: string; tc: TC }) {
  ...
  return (
    <div className="print-card" style={{ width: W, height: H, background: '#060614', position: 'relative', overflow: 'hidden', fontFamily: fontFamily ?? "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      {/* 本文 */}
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px 44px', textAlign: 'center' }}>
        {/* ブランドロゴ */}
        <div style={{ marginBottom: 10, filter: `drop-shadow(0 0 12px ${accent}80)` }}>
          <LogoIcon size={32} />
        </div>
        {/* キャッチコピー */}
        <p style={{ fontSize: 18, color: accent, fontWeight: 800, letterSpacing: '0.2em', margin: '0 0 8px', lineHeight: 1.2 }}>分身AI搭載名刺</p>
        <h3 style={{ fontSize: 18, fontWeight: 900, color: '#FFFFFF', margin: '0 0 10px', lineHeight: 1.3, letterSpacing: '-0.01em' }}>
          名刺をスキャンすると<br />私の分身AIと話せます
        </h3>
        {/* 機能リスト */}
        <BackFeatureList accent={accent} />
        {/* EKG ライン */}
        <div style={{ width: '100%', maxWidth: 320, marginBottom: 6 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: `${accent}18`, borderRadius: 20, padding: '6px 14px', border: `1px solid ${accent}35` }}>
          ...
          <span style={{ fontSize: 18, color: 'rgba(255,255,255,0.7)', fontWeight: 700, letterSpacing: '0.06em' }}>24時間 / 365日 / オンライン待機中</span>
        </div>
      </div>
...
function CardPreview({
  ...
  return (
    <div ref={wrapperRef} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, width: '100%' }}>
      <p style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.12em', color: '#A08068' }}>{label}</p>
      {/* スケール外枠（影・角丸） */}
      <div style={{
        borderRadius: 14,
        overflow: 'hidden',
...
export default function PrintCardPage() {
  ...
  type FrontProps = { card: BusinessCard; qrUrl: string; fontFamily?: string; logoUrl?: string; logoX?: number; logoY?: number }
  const FrontComponent: React.ComponentType<FrontProps> =
    layout === 'centered' ? (p) => <CenteredFront {...p} tc={tc} />
    : layout === 'split'  ? (p) => <SplitFront   {...p} tc={tc} />
    : layout === 'pulse'  ? (p) => <PulseFront   {...p} tc={tc} />
    : layout === 'free'   ? (p) => <FreeFront    {...p} tc={tc} printEls={printEls} design={design} />
    : standardMap[design]  // standard（デフォルト）
            ...
            <div style={{ display: 'flex', gap: 4, flex: 1 }}>
              {(Object.entries(layoutMeta) as [Layout, typeof layoutMeta[Layout]][]).map(([key, meta]) => (
                <button key={key} onClick={() => {
                  setLayout(key)
                  // 自由配置に切り替え時だけ printEls をリセット
                  if (key === 'free') { setPrintEls(DEFAULT_PRINT_ELS); setEditMode(true) }
                }} style={{
      ...
      <div className="no-print" style={{ padding: '24px 16px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 36 }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.1em', color: '#A08068' }}>{designMeta[design].label}</span>
        ...
        <CardPreview
          ...
          btnLabel="表面を保存"
          captureRef={frontRef}
          overlay={(layout === 'free' && editMode) ? (scale) => (
            /* 自由配置 × 編集モード: ドラッグハンドル */
            <>
              {logoUrl && (
              ...
              {(Object.entries(printEls) as [PrintElId, PrintEl][]).map(([id, el]) => (
                <div key={id} onPointerDown={(e) => handlePrintElDragStart(e, scale, id)} style={{ position: 'absolute', left: el.x - 2, top: el.y - 2, width: el.w + 4, height: el.h + 4, cursor: 'move', border: '2px dashed rgba(242,103,34,0.65)', borderRadius: 5, boxSizing: 'border-box', zIndex: 19, touchAction: 'none', userSelect: 'none' }}>
                  ...
                  <div onPointerDown={(e) => { e.stopPropagation(); handlePrintElResizeStart(e, scale, id) }} style={{ position: 'absolute', bottom: -7, right: -7, width: 14, height: 14, background: '#F26722', borderRadius: 3, cursor: 'se-resize', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 21, touchAction: 'none' }}>
            ...
            /* ロゴドラッグハンドル（全レイアウト共通） */
            <div onPointerDown={(e) => handleLogoDragStart(e, scale)} style={{ position: 'absolute', left: logoX - 3, top: logoY - 3, width: 96, height: 34, cursor: 'move', border: '2px dashed rgba(242,103,34,0.7)', borderRadius: 6, boxSizing: 'border-box', zIndex: 20, touchAction: 'none', userSelect: 'none' }}>
              <div style={{ position: 'absolute', top: -10, right: -10, width: 18, height: 18, borderRadius: '50%', background: '#F26722', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, pointerEvents: 'none' }}>✥</div>
            </div>
          ) : undefined}
        >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, width: '100%', maxWidth: 520 }}>
          <div style={{ textAlign: 'center' }}>
            <p style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.12em', color: '#A08068', marginBottom: 4 }}>QRコード素材</p>
